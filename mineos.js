      ...
      socket.on('data', function(data) {
        ...
        switch (query) {
          case 'modern':
            ...
            break;
          case 'legacy':
            var split = splitBuffer(data, 0x00a7);
            callback(null, {
              protocol: '',
              server_version: '',
              motd: buffer_to_ascii(split[0].slice(3)),
              players_online: parseInt(buffer_to_ascii(split[1])),
              players_max: parseInt(buffer_to_ascii(split[2]))
            });
            break;
        }
      });
